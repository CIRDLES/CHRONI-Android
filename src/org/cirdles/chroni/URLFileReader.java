...
//			mProgressDialog.dismiss();
	...
	// Handles downloads synchronously
	private class DownloadTaskSynchronous {
		private Context context;
		private boolean isCancelled;

		public DownloadTaskSynchronous(Context context) {
			this.context = context;
		}

		/*
		 * Begins synchronous download
		 */
		protected synchronized String executeSynchronousDownload(String... sUrl) {
		// take CPU lock to prevent CPU from going off if the user
		// presses the power button during download
		PowerManager pm = (PowerManager) context.getSystemService(Context.POWER_SERVICE);
		PowerManager.WakeLock wl = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, getClass().getName());
...
//						publishProgress((int) (total * 100 / fileLength));
		...
		protected synchronized void handlePostResult(String result) {
//			mProgressDialog.dismiss();
			if (result != null)
				Toast.makeText(context, "Download error: " + result,
						Toast.LENGTH_LONG).show();
		...
		protected synchronized void onCancelled(String result) {
			if(downloadMethod.contentEquals("igsn")){
				Toast.makeText(context, "Download error: You have specified an invalid IGSN",
					Toast.LENGTH_LONG).show();
			}else{
